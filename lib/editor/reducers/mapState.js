...
import {receivedRoutesShapefile, updateMapSetting, receivedStopsfile} from '../actions/map'
...
export const reducers = {
  ...
  'RECEIVED_ROUTES_SHAPEFILE' (
    ...
    console.log(':::: routesGeojson action.payload.geojson ',action.payload.geojson)
    return update(state, {
  ...
  'RECEIVED_STOPSFILE' (
    state: MapState,
    action: ActionType<typeof receivedStopsfile>
  ) {
    console.log(':::: stopsArrayObj action.payload.stopsArrayObj ',action.payload.stopsArrayObj)
    return update(state, {
      stopsArrayObj: {$set: action.payload.stopsArrayObj}
    })
  },
}
