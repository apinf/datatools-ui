...
export const receivedStopsfile = createAction(
  'RECEIVED_STOPSFILE',
  (payload: { stopsArrayObj: Object }) => payload
)
...
export type EditorMapActions = ActionType<typeof controlPointDragOrEnd> |
  ...
  ActionType<typeof receivedStopsfile>
 ...
 * Upload stop file
 *
 * @param  {Feed} feedSource
 * @param  {File} file Zip file of stop
 * @return  {Action}
 */
export function displayStopsfile (feedSource: Feed, file: File) {
  return function (dispatch: dispatchFn, getState: getStateFn) {
    const reader = new window.FileReader()
    reader.onload = e => {
      const stopsData = reader.result ? reader.result.split(/[\n,]+/) : [];
      const tempArray = [];
      while(stopsData && stopsData.length)
        tempArray.push(stopsData.splice(0,5))

      const stopsArrayObj = [];
      if(tempArray && tempArray.length) {
        tempArray.forEach((item, outerIndex) => {
          if(item && item.length && outerIndex > 0){
            let obj= {};
            item.forEach((innerItem, innerIndex) => {
              const key = tempArray[0][innerIndex];
              console.log(':: check ',key.includes('location_type'),' :: key ',key,' :: typeof ',typeof key)
              if(
                key.includes("location_type")
                || key.includes("stop_lat")
                || key.includes("stop_lon")
              ) {
                obj[key] = Number(innerItem);
                console.log(':: obj[key] ',obj[key])
              }
              else
                obj[key] = innerItem;
            });
            stopsArrayObj.push(obj);
          }
        });
        console.log(stopsArrayObj,' :: tempArray ',tempArray, ' :: feedSource ',feedSource)
        dispatch(saveEntity(feedSource.id, stopsArrayObj[0], 'stop'))
        // dispatch(receivedStopsfile({stopsArrayObj}))
      }
    }
