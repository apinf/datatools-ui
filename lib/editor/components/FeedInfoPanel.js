...
import {isValidZipFile, isValidTxtFile} from '../../common/util/util'
...
type Props = ContainerProps & {
  ...
  displayStopsfile: typeof mapActions.displayStopsfile,
  feedInfo: FeedInfo,
  feedSource: Feed,
...
export default class FeedInfoPanel extends Component<Props, State> {
  ...
  showUploadFileModalForStop = () => {
    const {displayStopsfile, feedSource} = this.props
    this.refs.selectFileModal.open({
      title: this.messages('uploadStopfile.title'),
      body: this.messages('uploadStopfile.body'),
      onConfirm: (files) => {
        console.log(':: files ',files)
        const file = files[0]
        if (isValidTxtFile(file)) {

          displayStopsfile(feedSource, file)
          return true
        } else {
          return false
        }
      },
      errorMessage: this.messages('uploadStopfile.error')
    })
  }
      ...
      <div>
        ...
        <div style={panelStyle}>
          ...
          <ButtonGroup>
            ...
            <Button
              onClick={this.showUploadFileModalForStop}>
              <Icon type='upload' />
            </Button>
            <Button
